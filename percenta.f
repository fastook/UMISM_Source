          ...
          DO I=1,NUMNP
            IF(ITYPE.EQ.1) THEN
              ...
              ELSE
                ...
                ENDIF
              ENDIF
c              IF(FRACT(I).LE. THRESH) THEN
c                PFRACT(I)=PFRACT(I)+DT
c              ENDIF
...
C              ENDIF
            ENDIF
C        PRINT *,I,REAL(PTEMP(I)),REAL(ATIME(I))
          ENDDO
          TOLD=TIME
        ENDIF
      GOTO 85
999   CONTINUE
      DO I=1,NUMNP
C        PFRACT(I)=PFRACT(I)/(TIME-TBASE)
C        PTEMP(I)=PTEMP(I)/(TIME-TBASE)
        PICE(I)=PICE(I)/(TIME-TBASE)
        IF(ATIME(I).NE.0.0) THEN
          PTEMP(I)=PTEMP(I)/ATIME(I)
          PFRACT(I)=PFRACT(I)/ATIME(I)
        ELSE
          PTEMP(I)=0.0
          PFRACT(I)=0.0
        ENDIF
