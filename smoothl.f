      ...
      DIMENSION X(NMAX),Y(NMAX),YY(NMAX)
      CHARACTER*80 JUNK
      READ(1,100) JUNK
      PRINT *,JUNK
100   FORMAT(A80)
...
99    CONTINUE
      ...
      PTS=50.
      SCALE=1.0
10    PRINT *,'INPUT SMOOTHING WINDOW',PTS
      READ(*,*,END=999) PTS
      PRINT *,'INPUT SCALING FACTOR',SCALE
      READ(*,*,END=999) SCALE
      IF(PTS.LE.0.) GOTO 999
      CALL NEWPAG
      CALL LINCLR(2)
      CALL MOVE(X(1),Y(1))
      DO I=2,N
      ...
      DO I=1,N
        YY(I)=Y(I)
      ENDDO
      CALL SMOOFT(YY,N,PTS)
      CALL SCALER(YY,N,SCALE)
      CALL MOVE(X(1),YY(1))
      CALL LINCLR(1)
      DO I=2,N
        CALL DRAW(X(I),YY(I))
