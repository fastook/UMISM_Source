...
C
     ...
     &          SLOPN(4,MXX),KODE(MXX),ITYPE(MXX)
      ...
      GRADZ=15./1000.
c      GRADZ=10./1000.
c from gisp core (doesnt yield sliding in scandinavian ice sheet)
c     GRADZ=23./3053.
      imelting=0
      imelted=0
      ifreezing=0
      ...
      if(icalc.eq.0) then
c----------------------------------------------
c ----this is for calculating fraction --------
      DO I=1,NUMNP
        IF(FRACT(I).LT.0. .AND. KODE(I).EQ.0) THEN
          AJUNK=ACCUM(X(I)*.001D0,Y(I)*.001D0,HTICE(I),SLOPN(1,I),
     &                          0.D0,AMASS(9),TEMP(I))
          THICK=HTICE(I)-DEPB(I)
          TBOT=TEMP(I)+GRADZ*THICK
c ----- a simple 1d steady state temperature solver
          call coltemp(AMASS,adot(i),temp(i),thick,tbot)
c -----------------
          tmin=min(tmin,tbot)
          tmax=max(tmax,tbot)
          IF(TBOT.GE.0) THEN
C           PRINT *,I,TBOT,TEMP(I),THICK
      ...
      print *,' melted   ',imelted
      print *,' freezing ',ifreezing
      print *,' frozen   ',ifrozen
      print *,real(tmin),real(tmax)
c----- end of fraction calculation -----------------------
c---------------------------------------------------------
      else
c----------------------------------------------
c ----this is for calculating flowa AND fract--------
      istep=numnp/5
      DO I=1,NUMNP
...
c ----- a simple 1d steady state temperature solver
          if(thick.eq.0.) then
            ...
            call coltemp1(AMASS,adot(i),temp(i),thick,tbot,AEFF)
c************************************************
c********** FUDGE !! ****************************
            AEFF=AEFF*AMASS(10)
C************************************************
...
c -----------------
          ...
          IF(TBOT.GE.0 .and. ifract.eq.1) THEN
C           PRINT *,I,TBOT,TEMP(I),THICK
            IC1=IC1+1
            FRACT(I)=FRACT(I)-.1
            IF(FRACT(I).LT.-1.) then
...
c----- end of flowa calculation -----------------------
c---------------------------------------------------------
      endif
      RETURN
      END
c**************************************************************
      subroutine coltemp1(AMASS,adot,tsurf,thick,tbot,aeff)
c******************************************
...
c           
c          
c      call grstrt(600,600)
      tmin=-60.
      tmax=60.
c      call window(real(tmin),real(tmax),real(xmin),real(xmax))
c      call linclr(1)
c      call move(0.,real(xmin))
c      call draw(0.,real(xmax))
c      call mrkclr(2)
      wwww(npt)=0. 
...
c...........loop back to beginning after recalculating vertical velocity
        ...
        enddo                                                        
c        call linclr(1)                                             
c        call move(real(told(1)),real(xxx(1)))                           
c        do i=2,npt                                                  
c          call draw(real(told(i)),real(xxx(i)))                         
c        enddo                                                        
c        call linclr(2)                                             
c        call move(real(tttt(1)),real(xxx(1)))                           
c        do i=2,npt                                                  
c          call draw(real(tttt(i)),real(xxx(i)))                         
c        enddo                                                        
        diff=0d0
        do i=1,npt
...
c**********************************
      subroutine coltemp(AMASS,adot,tsurf,thick,tbot)
c******************************************
c this solves for steady state vertical temperature profile
c with solved for basal temperature
c if basal temp > pressure melting, will
