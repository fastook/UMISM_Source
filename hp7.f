...
C     REWIND 1
      ISKIP=0
10    READ(1,1000,END=9999) HED,NUMNP,NUMEL,NUMCOL,NUMLEV,NUMGBC,NDT,
     &INTER,DT
      IF(IPASS.EQ.0) THEN
      WRITE(*,1010) HED
...
231   CONTINUE
     ...
     &ADOT,FRACT,PSURF,BED,FLOWA,SLDGB
      IF(IPLOT.EQ.0) ZZ(I)=HT
      IF(IPLOT.EQ.1) ZZ(I)=BED
      IF(IPLOT.EQ.2) ZZ(I)=HT-BED
      IF(IPLOT.EQ.5) ZZ(I)=ADOT
      IF(IPLOT.EQ.6) ZZ(I)=FLOWA
      IF(IPLOT.EQ.7) ZZ(I)=SLDGB
      IF(IPLOT.EQ.8) ZZ(I)=FRACT
      IF(IPLOT.EQ.9) ZZ(I)=PSURF
      XMAX=AMAX1(X(N),XMAX)
      XMIN=AMIN1(X(N),XMIN)
      YMAX=AMAX1(Y(N),YMAX)
      YMIN=AMIN1(Y(N),YMIN)
...
580   CONTINUE
      ...
      YY(LM(J))=(RSCALE*ZZ(LM(J))+Y(LM(J))*SINS)*RSCL
      IQ=LM(J)
590   CONTINUE
...
c     IF(ISKIP.EQ.1) GOTO 1234
      ISKIP=1
C     CALL LABEL1(3030,2800,IPLOT,HED)
      CALL TXTCLR(0)
      CALL MOVE(10.,95.)
      CALL TEXT(80,HED)
      CALL MOVE(10.,92.5)
...
C     CALL GRSTOP
      CALL MAKCUR
1235  continue
c     WRITE(*,*) 'INPUT 1 TO CONTINUE'
c     READ(*,*,END=9999) IJUNK
c     IF(IJUNK.LT.0) GOTO 9999
C     GOTO 10
      GOTO 1
999   CONTINUE
C     WRITE(*,*) '       '
