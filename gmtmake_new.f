...
C      DIMENSION X(44,40),Y(44,40),Z(44,40)
      ...
      DIMENSION CLAT(NMAX),CLONG(NMAX)
      ...
      COMMON/TRIG/RKMPDEG,RADPDEG,DEGPRAD,rlongavg
...
c       READ(1,200) N,KODE,XX(NUM),YY(NUM),HTICE(NUM),
     ...
     &              TBED,itype,AFUDGE,GEOFLUX(NUM),ABLAT,
     &              CLAT(N),CLONG(N),CONTRIB(N)
        rlatcent=rlatcent+CLAT(num)
        rloncent=rloncent+CLONG(num)
        xcent=xcent+xx(num)
        ycent=ycent+yy(num)
        RLAT=CLAT(N)
        RLONG=CLONG(N)
...
C CHANGE PLOTTED VARIABBLE HERE
        IF(IPLOT.EQ.0) ZZ(N)=HTICE(N)*1
        IF(IPLOT.EQ.1) ZZ(N)=BDROCK(N)*1
        IF(IPLOT.EQ.2) ZZ(N)=ADOT*100 ! cm/yr
        IF(IPLOT.EQ.3) ZZ(N)=FRACT
        IF(IPLOT.EQ.4) ZZ(N)=FLOWA
        IF(IPLOT.EQ.5) ZZ(N)=HTICE(N)-PSURF
        IF(IPLOT.EQ.6 .OR. IPLOT.EQ.7) THEN
          ZZ(N)=max(0.,HTICE(N)-BDROCK(N))*1
...
c         ENDIF
        ...
        IF(IPLOT.EQ.11) ZZ(N)=TBED+273.16
        IF(IPLOT.EQ.12) ZZ(N)=AFUDGE
        IF(IPLOT.EQ.17) ZZ(N)=WTHICK
        IF(IPLOT.EQ.17) ZZ(N)=max(0.0,WTHICK)
        IF(IPLOT.EQ.18) ZZ(N)=BMELT*1000.
c ...   WARNING FREEZE ON IS SUPPRESSED ...
        IF(IPLOT.EQ.18) then
          if(bmelt.gt.0) then
            ZZ(N)=MAX(0.0,BMELT*1000.)
          else
            zz(n)=0.01
            zz(n)=0
          ENDIF
...
C CALCULATE D(PSI)/DX...EQUATION (5.3.5)                                
          ...
          DO ii=1,NNODE                                                  
            ...
            RLAT=RLAT+CLAT(LM(ii))*PSI(ii)
            RLONG=RLONG+CLONG(LM(ii))*PSI(ii)
...
c         VEL=UMAG        ! m/yr
          ...
          WRITE(9,123) CLONG(NUM),CLAT(NUM),VEL
        ...
        ELSEIF(IPLOT.EQ.16) THEN
          ...
          SLOPE = DH                        ! m/m
          SLOPE = DH*1000                   ! m/km
          SLOPE = ATAN(DH) * 180./3.1415927 ! degrees
          WRITE(9,123) RLONG,RLAT,SLOPE
          WRITE(10,*) XPLOT*.001,YPLOT*.001,SLOPE
...
c===============================================
      ...
      COMMON/TRIG/RKMPDEG,RADPDEG,DEGPRAD,rlongavg
      y= 1000*rkmpdeg*rlat
      x= 1000*rkmpdeg*cos(rlat*radpdeg)*(rlong-rlongavg)
c ... for arsia ...
c     x= 1000*rkmpdeg*cos(rlat*radpdeg)*(rlong+127.5)
c     x= 1000*rkmpdeg*cos(rlat*radpdeg)*(rlong+115.0)
c ... for gusev ...
c     x= 1000*rkmpdeg*cos(rlat*radpdeg)*(rlong-175.0)
c ... for deteronilus ...
c     x= 1000*rkmpdeg*cos(rlat*radpdeg)*(rlong-22.0)
      END
c===============================================
      ...
      COMMON/TRIG/RKMPDEG,RADPDEG,DEGPRAD,rlongavg
      rlat=y/rkmpdeg
c     rlong=-127.5+x*0.001/rkmpdeg/cos(rlat*radpdeg)
      rlong= rlongavg+x*1/rkmpdeg/cos(rlat*radpdeg)
      END
