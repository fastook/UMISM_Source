C READS FROM STANDARD OUT3 AND PLOTS CONTOURS DIRECTLY
      include "parameter.h"
      PARAMETER(NMAX=MAXNUM,NCOLOR=15,NPLOT=23000)
...
c        IF(IPLOT.EQ.14) RLEVSP=.01
      ...
      ENDIF
      NLINE=0
      inquire(file='fort.11',exist=file)
      if(file) then
...
c      ENDIF
      ...
      DO 910 I=3,NSEG
        ...
        IF(IPAUSE.EQ.-1) then
          if(iflush) call gflush
          CALL GETUIN(-1,'CONTINUE?',1,INARAY,IGOT)
        endif
