...
c and for the time-dependent modified matrices, and also the gauss-seidel
...
c-----------------------------------------------------------------------
      ...
      DIMENSION GK(M3,NZ),GC(M3,NZ),GF(M3)
...
C ....TIME DEPENDENT CASE AND VARYING LOAD .......
      CALL PTIMEDEP(N3,NZ,NNODE,GK0,GC0,GK,GC,GF,KA,KZ,
     &              WWW,DELT)
C ......................................................
...
c        WRITE(7,1000) ' TIME BEFORE PGAUSEID ',ETIME(TB),DTIME(TB)!
        CALL PGAUSEID(N3,NZ,NNODE,1.D-6,GK,KA,GF,WWW)                   !
c        WRITE(7,1000) ' TIME AFTER PGAUSEID ',ETIME(TB),DTIME(TB) !
C ................................................................!
C     ELSE
...
C      II=1
C      DO I=1,NNODE,3
C        WDIFF=WWW(I)-WWWSAVE(II)
C        WMIN=MIN(WMIN,WDIFF)
C        WMAX=MAX(WMAX,WDIFF)
C        WTOT=WTOT+WDIFF
C        II=II+1
C      ENDDO
      write(*,*) '*******************************************'
...
1002  FORMAT(A,/,(1P2E13.6))
      END
C=============================================================
      SUBROUTINE PASSMBGK(N3,NZ,GK,GC,KA,KZ,EK,EC,N,NODE)
      IMPLICIT REAL*8(A-H,O-Z)
      DIMENSION GK(N3,NZ),GC(N3,NZ)
...
C.........ASSEMBLE GLOBAL STIFFNESS MATRIX GK
            IF(I.EQ.J) THEN
              GK(I,1)=GK(I,1)+EK(L,M)
              GC(I,1)=GC(I,1)+EC(L,M)
              KA(I,1)=I
            ELSE
              DO K=2,KZ(I)
                IF(KA(I,K).EQ.J) THEN
                  GK(I,K)=GK(I,K)+EK(L,M)
                  GC(I,K)=GC(I,K)+EK(L,M)
                  GOTO 99
                ENDIF
              ENDDO
              KZ(I)=KZ(I)+1
...
c          PAUSE
        ENDIF
C..................................
        CALL PASSMBGK(N3,NZ,GK,GC,KA,KZ,EK,EC,12,LLM)
C..................................
C        DO I=1,NNODE
C          PRINT 1000,(GK(I,J),J=1,NNODE)
C        ENDDO
C        PAUSE
C..................................
      ENDDO
...
1033  FORMAT(1X,'NAME LOAD')
      END
C=============================================================
      SUBROUTINE PTIMEDEP(N3,NZ,NNODE,GK0,GC0,GK,GC,GF,KA,KZ,
     &                    WWW,DELT)
      IMPLICIT REAL*8(A-H,O-Z)
